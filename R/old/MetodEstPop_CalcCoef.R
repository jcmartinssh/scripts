...
library(tidyverse)
library(data.table)
# library(crsuggest) #para selecionar o crs de projeção e cálculo de áreas
...
# teste_crs <- suggest_crs(SetoresBH)
...
agregado <- BH_Calculo %>%
  ...
  select(CD_GEOCODI, CD_GEOCODS, CD_GEOCODD, CD_GEOCODM, N_SubD, N_Dist, N_Mun, ID_UNICO, POP, V002,  Vaz, UrPd, UrDs, NRes, AreUrDsSet, AreUrPdSet, AreNResSet, AreaVazSet)

agregado <- data.table(agregado)
...
fnopt_dpp <- function(par, data) {
  data[, POPEstTrecho := V002 * ((UrDs * par[1]) + (UrPd * par[2]) + (Vaz * par[3]) + (NRes * par[4])) /
         ((AreUrDsSet * par[1]) + (AreUrPdSet * par[2]) + (AreaVazSet * par[3]) + (AreNResSet * par[4]))
       ][, .(POPEst = sum(POPEstTrecho), POPGde = first(POP)),
         by = "ID_UNICO"][, Erro := abs(POPGde - POPEst)
                          ][, .(total = sum(Erro))]
}
...
for (i in SubDists) {
  selecao <- agregado[N_SubD == 1 & CD_GEOCODS == i]
...
for (i in Dists) {
  selecao <- agregado[N_Dist == 1 & CD_GEOCODD == i]
  amostra <- n_distinct(selecao$ID_UNICO)
  result <- optim(par = par.ini, fn = fnopt_dpp, data = selecao, method = "L-BFGS-B", lower = 10^-9)
...
for (i in Munic) {
  selecao <- agregado[N_Mun == 1 & CD_GEOCODM == i]
  amostra <- n_distinct(selecao$ID_UNICO)
  result <- optim(par = par.ini, fn = fnopt_dpp, data = selecao, method = "L-BFGS-B", lower = 10^-9)
