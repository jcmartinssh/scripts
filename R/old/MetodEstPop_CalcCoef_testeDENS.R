...
SubDists <- c(31578070500, 31062006066, 31578071000, 31062006064)

amostra <- agregado %>%
  filter(CD_GEOCODS %in% SubDists)
...
fnopt_dpp <- function(data, par) {
  data %>%
    mutate(POPEstTrecho = DenM_set * ((UrDs * par[1]) + (UrPd * par[2]) + (Vaz * par[3]) + (NRes * par[4]))) %>%
    group_by(ID_UNICO) %>%
    summarise(POPEst = sum(POPEstTrecho))
}
...
# 
# # fit linear model
# linear_model <- lm(y ~ x / z, data=df)
# 
# # view summary of linear model
# summary(linear_model)


lm <- for (i in SubDists) {
  selecao <- filter(agregado, N_SubD == 1 & CD_GEOCODS %in% SubDists)
  # amostra <- n_distinct(selecao$ID_UNICO)
  result <- lm(first(selecao$POP) ~ sum(selecao$DenM_set * ((selecao$UrDs) + (selecao$UrPd) + (selecao$Vaz) + (selecao$NRes))))
