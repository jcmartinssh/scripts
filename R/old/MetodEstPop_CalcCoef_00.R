...
BH_Calculo <- GradeBH %>%
  ...
  mutate(set_amostra = ifelse(NC_Cont >1, TRUE, FALSE), # será que esse filtro não está prejudicando? não teria que ser as grades inteiras?
         POPCalcInter = ifelse(set_amostra == TRUE, 0, V002 * Area_Inter / Area_Orig)) %>% # isso aqui tá dando merda
  group_by(ID_UNICO) %>%
...
agregado <- BH_Calculo %>%
  pivot_wider(id_cols = c(CodGrupSet, set_amostra, CD_GEOCODI, CD_GEOCODS, CD_GEOCODD, CD_GEOCODM, N_SubD, N_Dist, N_Mun, ID_UNICO, DOM_OCU, POP, POP_aj, CD_GEOCODI, V002, POPCalc, AreUrDsSet, AreUrPdSet, AreNResSet, AreaVazSet),
              names_from = ClasseUrb,
              values_from = Area_Inter,
              values_fn = sum) %>%
  replace_na(list(Vaz = 0, UrPd = 0, UrDs = 0, NRes = 0)) %>%
  # filter(set_amostra == TRUE) %>%
  select(CD_GEOCODS, CD_GEOCODD, CD_GEOCODM, N_SubD, N_Dist, N_Mun, ID_UNICO, POP_aj, V002,  Vaz, UrPd, UrDs, NRes, AreUrDsSet, AreUrPdSet, AreNResSet, AreaVazSet)

end_time <- Sys.time()
tempoProcTab <- end_time - start_time

## análise estatística do agregado

selecao <- filter(agregado)

agregado %>%
  group_by(ID_UNICO) %>%
  filter(UrDs > 0) %>%
  summarise(POP_aj = sum(POP_aj), UrDs = sum(UrDs), UrPd = sum(UrPd), NRes = sum(NRes), Vaz = sum(Vaz)) %>% 
  ggplot(aes(y = POP_aj)) +
  geom_point(aes(x = UrDs), color = "red")
...
agregado %>%
  ...
  summarise(POP_aj = sum(POP_aj), UrDs = sum(UrDs), UrPd = sum(UrPd), NRes = sum(NRes), Vaz = sum(Vaz)) %>% 
  ggplot(aes(y = POP_aj)) +
  geom_point(aes(x = UrPd), color = "orange")
...
agregado %>%
  ...
  summarise(POP_aj = sum(POP_aj), UrDs = sum(UrDs), UrPd = sum(UrPd), NRes = sum(NRes), Vaz = sum(Vaz)) %>% 
  ggplot(aes(y = POP_aj)) +
  geom_point(aes(x = NRes), color = "yellow")
...
agregado %>%
  ...
  summarise(POP_aj = sum(POP_aj), UrDs = sum(UrDs), UrPd = sum(UrPd), NRes = sum(NRes), Vaz = sum(Vaz)) %>% 
  ggplot(aes(y = POP_aj)) +
  geom_point(aes(x = Vaz), color = "cyan")
