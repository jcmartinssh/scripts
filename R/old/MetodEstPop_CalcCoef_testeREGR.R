...
amostra_min <- 20
# par.ini <- c(1, 1, 1, 1)
start.ini <- list(coefDS = 4, coefPD = 2, coefVZ = 0.05, coefNR = 0.05)
...
BH_Calculo <- GradeBH %>%
  ...
  mutate(NC_Cont = n_distinct(ClasseUrb),
         DenM_set = V002 / Area_Orig) %>%
  ungroup() %>%
  left_join(SetoresBH, by = "CD_GEOCODI") %>%
  mutate(CD_GEOCODS = CD_GEOCODS.y, CD_GEOCODD = CD_GEOCODD.y, CD_GEOCODM = CD_GEOCODM.y) %>%
  select("CodGrupSet", "ID_UNICO", "DOM_OCU", "POP", "CD_GEOCODI", "NC_Cont", "CD_GEOCODS", "CD_GEOCODD", "CD_GEOCODM", "V002", "DenM_set", "DenM_grd", "ClasseUrb", "Area_Orig", "Area_grade", "AreUrDsSet", "AreUrPdSet", "AreNResSet", "AreaVazSet", "Area_Inter") %>%
  group_by(ID_UNICO) %>%
...
teste <- BH_Calculo %>%
  filter(Area_Inter < 10^-3)
    
agregado <- BH_Calculo %>%
  pivot_wider(id_cols = c(CodGrupSet, CD_GEOCODI, CD_GEOCODS, CD_GEOCODD, CD_GEOCODM, N_SubD, N_Dist, N_Mun, ID_UNICO, DOM_OCU, POP, CD_GEOCODI, V002, DenM_set, DenM_grd, Area_grade, AreUrDsSet, AreUrPdSet, AreNResSet, AreaVazSet, grd_amostra),
              names_from = ClasseUrb,
              values_from = Area_Inter,
              values_fn = sum) %>%
  replace_na(list(Vaz = 0, UrPd = 0, UrDs = 0, NRes = 0)) %>%
  filter(grd_amostra == TRUE) %>%
  select(CD_GEOCODS, CD_GEOCODD, CD_GEOCODM, N_SubD, N_Dist, N_Mun, ID_UNICO, POP, V002, DenM_set, DenM_grd, Area_grade, Vaz, UrPd, UrDs, NRes, AreUrDsSet, AreUrPdSet, AreNResSet, AreaVazSet)
...
# fnopt_dpp <- function(data, coefDS, coefPD, coefVZ, coefNR) {
#   data %>%
#     group_by(ID_UNICO) %>%
#     summarise(POP = first(POP), result = list(nls(POP ~ I(sum(V002 * ((UrDs * coefDS) + (UrPd * coefPD) + (Vaz * coefVZ) + (NRes * coefNR)) / 
#                              ((AreUrDsSet * coefDS) + (AreUrPdSet * coefPD) + (AreaVazSet * coefVZ) + (AreNResSet * coefNR))))), data = cur_data(), start = list(coefDS = 1, coefPD = 1, coefVZ = 1, coefNR = 1) ))
# }

form_t <- formula(DenM_grd ~ sum(DenM_set * ((UrDs * coefDS) + (UrPd * coefPD) + (Vaz * coefVZ) + (NRes * coefNR))) / Area_grade)

form <- formula(POP ~ I(sum(V002 * ((UrDs * coefDS) + (UrPd * coefPD) + (Vaz * coefVZ) + (NRes * coefNR)) / 
                              ((AreUrDsSet * coefDS) + (AreUrPdSet * coefPD) + (AreaVazSet * coefVZ) + (AreNResSet * coefNR)))))

fnopt_dpp <- function(dados, coefDS, coefPD, coefVZ, coefNR)

fnopt_dpp <- function(dados, coefDS, coefPD, coefVZ, coefNR) {
  dados %>%
    summarise(POPest = V002 * ((UrDs * coefDS) + (UrPd * coefPD) + (Vaz * coefVZ) + (NRes * coefNR)) / 
                ((AreUrDsSet * coefDS) + (AreUrPdSet * coefPD) + (AreaVazSet * coefVZ) + (AreNResSet * coefNR)))
}

start_time <- Sys.time()

selecao <- filter(agregado, N_SubD == 1, CD_GEOCODS == 31578070500)

teste <- fnopt_dpp(selecao, form, coefDS, coefPD, coefVZ, coefNR)
teste <- nlsList(POP ~ fnopt_dpp(selecao, coefDS, coefPD, coefVZ, coefNR) | ID_UNICO, data = selecao, start = list(coefDS = 4, coefPD = 2, coefVZ = 0.01, coefNR = 0.01))
    
amostra <- n_distinct(selecao$ID_UNICO)

# result <- nls(POP ~ fnopt_dpp(selecao, coefDS, coefPD, coefVZ, coefNR), dados = selecao, start = start.ini)
...
#   amostra <- n_distinct(selecao$ID_UNICO)
#   result <- nls(POP ~ fnopt_dpp(dados, par), data = selecao, start = par.ini, trace = TRUE)
#   SubD_coef_dpp[[i]] = data.frame(i)
#   SubD_coef_dpp[[i]]$CD_GEOCODS = i
#   SubD_coef_dpp[[i]]$coefDS_Sd = result$par[1]
#   SubD_coef_dpp[[i]]$coefPD_Sd = result$par[2]
...
# 
# 
# #st_write(CoefsFinal, dsn = "W:/DGC_ACERVO_CGEO/PROJETOS_EM_ANDAMENTO/Cemaden/BOLSISTAS/Joaquim/_GPKG/Piloto_BH.gpkg", layer = "CoefsFinal_r", append = FALSE)
# 
