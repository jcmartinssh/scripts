...
Coefs_pop <- Coefs_SubD_pop %>%
  ...
  left_join(Coefs_Dist_pop, by = "CD_GEOCODD") %>%
  left_join(Coefs_Mun_pop, by = "CD_GEOCODM") %>%
  mutate(pop_coefDS = ifelse(!is.na(pop_coefDS_SubD) & amostra_SubD >= amostra_min, pop_coefDS_SubD, 
                             ifelse(!is.na(pop_coefDS_Dist) & amostra_Dist >= amostra_min, pop_coefDS_Dist, pop_coefDS_Mun)),
         pop_coefPD = ifelse(!is.na(pop_coefPD_SubD) & amostra_SubD >= amostra_min, pop_coefPD_SubD, 
                             ifelse(!is.na(pop_coefPD_Dist) & amostra_Dist >= amostra_min, pop_coefPD_Dist, pop_coefPD_Mun)),
         pop_coefVZ = ifelse(!is.na(pop_coefVZ_SubD) & amostra_SubD >= amostra_min, pop_coefVZ_SubD, 
                             ifelse(!is.na(pop_coefVZ_Dist) & amostra_Dist >= amostra_min, pop_coefVZ_Dist, pop_coefVZ_Mun)),
         pop_coefNR = ifelse(!is.na(pop_coefNR_SubD) & amostra_SubD >= amostra_min, pop_coefNR_SubD, 
                             ifelse(!is.na(pop_coefNR_Dist) & amostra_Dist >= amostra_min, pop_coefNR_Dist, pop_coefNR_Mun))) %>%
  select(CD_GEOCODS, CD_GEOCODM, pop_coefDS, pop_coefPD, pop_coefVZ, pop_coefNR)

Coefs_dom <- Coefs_SubD_dom %>%
  mutate(CD_GEOCODD = str_sub(CD_GEOCODS, start = 1, end = 9), CD_GEOCODM = str_sub(CD_GEOCODS, start = 1, end = 7)) %>%
  left_join(Coefs_Dist_dom, by = "CD_GEOCODD") %>%
  left_join(Coefs_Mun_dom, by = "CD_GEOCODM") %>%
  mutate(dom_coefDS = ifelse(!is.na(dom_coefDS_SubD) & amostra_SubD >= amostra_min, dom_coefDS_SubD, 
                             ifelse(!is.na(dom_coefDS_Dist) & amostra_Dist >= amostra_min, dom_coefDS_Dist, dom_coefDS_Mun)),
         dom_coefPD = ifelse(!is.na(dom_coefPD_SubD) & amostra_SubD >= amostra_min, dom_coefPD_SubD, 
                             ifelse(!is.na(dom_coefPD_Dist) & amostra_Dist >= amostra_min, dom_coefPD_Dist, dom_coefPD_Mun)),
         dom_coefVZ = ifelse(!is.na(dom_coefVZ_SubD) & amostra_SubD >= amostra_min, dom_coefVZ_SubD, 
                             ifelse(!is.na(dom_coefVZ_Dist) & amostra_Dist >= amostra_min, dom_coefVZ_Dist, dom_coefVZ_Mun)),
         dom_coefNR = ifelse(!is.na(dom_coefNR_SubD) & amostra_SubD >= amostra_min, dom_coefNR_SubD, 
                             ifelse(!is.na(dom_coefNR_Dist) & amostra_Dist >= amostra_min, dom_coefNR_Dist, dom_coefNR_Mun))) %>%
...
# ListaMunMap <- ListaMunMap[1:2]
# ListaMunNMap <- ListaMunNMap[1:2]
...
TabelaCalcFinal <- na.omit(TabelaCalcFinal, cols = c("CD_GEOCODI", "ID_UNICO"))
...
# fwrite(TabelaCalcVar, file = "C:/ACELERADOR/EstPop/TabelaCalcVar.csv", append = FALSE, quote = TRUE, sep = ";", dec = ".")
# fwrite(TabelaCalcFinal, file = "C:/ACELERADOR/EstPop/TabelaCalcFinal.csv", append = FALSE, quote = TRUE, sep = ";", dec = ".")
